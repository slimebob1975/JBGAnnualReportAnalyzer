...
def _result(metrics=None, **kwargs):
    """Metric names contain spaces, so they are passed as a dict."""
    metrics = dict(metrics or {}, **kwargs)
    return {
...
def test_identical_readings_produce_no_findings():
    first = _result({"Summa tillgångar": 63853, "Summa eget kapital": 16339})
    a = _analyzer([_result({"Summa tillgångar": 63853, "Summa eget kapital": 16339})])
    assert a._check_extraction_stability(first, ["t"], 2025, "m", "x.pdf") == []


def test_a_changed_value_is_flagged():
    first = _result({"Summa tillgångar": 63853})
...
def test_the_first_reading_is_kept():
    ...
    assert first[fund]["2025"]["Summa tillgångar"]["värde"] == 63853
...
def test_only_the_unstable_metric_is_flagged():
    first = _result({"Summa tillgångar": 63853, "Summa skulder": 42401})
    a = _analyzer([_result({"Summa tillgångar": 16026, "Summa skulder": 42401})])
    findings = a._check_extraction_stability(first, ["t"], 2025, "m", "x.pdf")
    assert [f.metrics[0] for f in findings] == ["Summa tillgångar"]
...
def test_formatting_differences_are_not_instability(first_value, second_value):
    first = _result({"Summa tillgångar": first_value})
    a = _analyzer([_result({"Summa tillgångar": second_value})])
    assert a._check_extraction_stability(first, ["t"], 2025, "m", "x.pdf") == []
...
def test_findings_follow_the_fund_rename():
    ...
    a.stability_findings = [
        validation.Finding(fund="Livs", year="2025", rule="Instabil avläsning",
                           message="m", metrics=["Summa tillgångar"])
    ]
...
def test_serialised_findings_reach_the_result_file():
    finding = validation.Finding(
        ...
        message="Två avläsningar gav olika värden", metrics=["Summa tillgångar"],
    )
