...
def test_missing_metrics_lists_what_is_absent():
    a = _analyzer()
    present = [n for n in ALL_METRICS if n not in ("Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav", "Summa skulder")]
    missing = a._missing_metrics(_result(present))
    assert missing == [n for n in ALL_METRICS if n in ("Summa skulder", "Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav")]


def test_null_valued_metrics_count_as_missing():
    a = _analyzer()
    result = _result(ALL_METRICS)
    result["Kommunalarbetarnas"]["2024"]["Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav"]["värde"] = None
    assert a._missing_metrics(result) == ["Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav"]
...
def test_second_pass_recovers_a_missed_metric():
    ...
    def fake(system_prompt, request_text, model="", response_schema=None, **kwargs):
        ...
        return json.dumps({
            ...
            "nyckeltal": [{"namn": "Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav", "värde": 4211, "källa": "Not 7",
    ...
    assert metrics["Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav"]["värde"] == 4211
    # the schema makes it structurally impossible to answer about anything else
    assert set(seen["enum"]) == {"Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav", "Summa skulder"}
    ...
    assert '"Nyckeltal": "Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav"' in seen["prompt"]


def test_recovered_values_are_tagged():
    a = _analyzer()
    result = _result([n for n in ALL_METRICS if n not in ("Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav", "Summa skulder")])
    a._make_openai_api_call = lambda *args, **kwargs: json.dumps({
        "kassa": "K", "år": 2024,
        "nyckeltal": [{"namn": "Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav", "värde": 1, "källa": "s",
                       "säkerhet": 0.8, "kommentar": "Hittad i not 7."}],
    }, ensure_ascii=False)
    a._second_pass_for_missing(result, ["t"], the_year=2024, model="m")

    comment = result["Kommunalarbetarnas"]["2024"]["Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav"]["kommentar"]
...
def test_first_pass_values_are_never_overwritten():
    ...
    a._make_openai_api_call = lambda *args, **kwargs: json.dumps({
        "kassa": "K", "år": 2024,
        "nyckeltal": [
            {"namn": "Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav", "värde": 1, "källa": "s", "säkerhet": 0.8, "kommentar": "c"},
...
def test_values_are_grafted_onto_the_existing_fund_and_year():
    ...
    a._make_openai_api_call = lambda *args, **kwargs: json.dumps({
        ...
        "nyckeltal": [{"namn": "Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav", "värde": 7, "källa": "s",
    ...
    assert result["Kommunalarbetarnas"]["2024"]["Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav"]["värde"] == 7
...
def test_stops_early_once_nothing_is_missing():
    ...
    def fake(system_prompt, request_text, model="", response_schema=None, **kwargs):
        ...
        return json.dumps({
            "kassa": "K", "år": 2024,
            "nyckeltal": [
                {"namn": "Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav", "värde": 1, "källa": "s",
                 "säkerhet": 0.8, "kommentar": "c"},
...
def test_a_failed_second_pass_leaves_the_result_intact():
    a = _analyzer()
    present = [n for n in ALL_METRICS if n != "Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav"]
    result = _result(present)
...
def test_unparsable_second_pass_response_is_survived():
    ...
    a._second_pass_for_missing(result, ["t"], the_year=2024, model="m")
    assert "Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav" not in result["Kommunalarbetarnas"]["2024"]


# ------------------------------------------------------------------ schema
def test_restricted_schema_is_still_strict():
    a = _analyzer()
    built = a._response_schema(["Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav", "Summa skulder"])
    assert built["strict"] is True
    item = built["schema"]["properties"]["nyckeltal"]["items"]
    assert item["properties"]["namn"]["enum"] == ["Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav", "Summa skulder"]
...
def test_schemas_are_cached_per_metric_set():
    ...
    part_a, part_b = a._response_schema(["Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav"]), a._response_schema(["Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav"])
...
def test_second_pass_instruction_permits_omission():
    ...
    text = schema.describe_for_second_pass(["Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav"])
    assert "Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav" in text
...
def test_a_single_missing_metric_does_not_trigger_a_pass():
    ...
    result = _result([n for n in ALL_METRICS if n != "Not till Fordringar felaktig arbetslöshetsersättning: Årets avstående från återkrav"])
    a._second_pass_for_missing(result, ["text"], the_year=2024, model="gpt-5.2")
    assert calls == []
